/* HyperE.java
 ...
 06/03 - Sasha
       ...
       - now use Get*PriceWithCapital functions for the banner price
 06/03 - Madan
                     ...
                     Banner[] bannerList, Category categories)
{
  ...
  try {
    ...
    for (h=0; h < MiscFunc.NUM_ACCESSES; h++) {
      ...
      switch (model_id) {
        case ECONOMY_ID:
          own_price = bannerList[currentSite].GetPrice();
          break;
        case INTERMEDIATE_ID:
          own_price = bannerList[currentSite].GetIntPriceWithCapital();
          break;
        case HYPERECONOMY_ID:
          own_price = bannerList[currentSite].GetHyperEPrice();
          break;
      }
            ...
            bannerList[currentSite].numLeft > 0 )
        {
          switch (model_id) {
            ...
            case ECONOMY_ID:
              ...
              price = (double) bannerList[bannerToShow].GetPrice();
            ...
            case INTERMEDIATE_ID:
              ...
              price = bannerList[bannerToShow].GetIntPrice();
              sort_factor =  utility * (1.0 + own_price - price);
              break;
            case HYPERECONOMY_ID:
              utility = (double) (
                  ...
                  (bannerList[bannerToShow].siteVsBanImp[currentSite] + 1.0 /
                   ((siteList[currentSite].siteVsCatClick[matchingCategory] +
                    ...
                    (bannerList[bannerToShow].siteVsBanImp[currentSite] +
                     ...
                     ((categories.catClicks[matchingCategory] + 1.0 ) /
                      (categories.catDisplays[matchingCategory] + 1.0 / MiscFunc.GLOBAL_CLICK_RATE))))));
              price = (double) bannerList[bannerToShow].GetHyperEPrice();
    ...
    try {
      ...
      bannerOutput.println
         ("ID   numLeft     bias   S.Rate    Clicks    Displays   Price  capital   Birth     Death       #catIn  categoriesIn");
            for (i = 0; i < MiscFunc.NUM_BANNERS; i++) {
              price= 0.0;
              switch (model_id) {
                  case ECONOMY_ID:
                       price = (double) bannerList[i].GetPrice();
                       break;
                  case INTERMEDIATE_ID:
                       price = (double) bannerList[i].GetIntPrice();
                       break;
                  case HYPERECONOMY_ID:
                       price = (double) bannerList[i].GetHyperEPrice();
               }
              ...
              bannerOutput.print(i + "   \t" + bannerList[i].numLeft +
                ...
                "\t"   + (float) bannerList[i].numClicksTotal   +
                "   \t" + (int)   bannerList[i].numDisplaysTotal +
                "  \t" + (float) bannerList[i].GetPrice() +
                "\t"   + (float) bannerList[i].capital +
