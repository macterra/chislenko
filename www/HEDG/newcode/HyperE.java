/* HyperE.java
 ...
 06/11 - Sasha
       - Changes to hypereconomy utility calculation
 06/10 - Madan
       - Changed formula used in variable site frequency
       - Added Category.* that reports category data
 06/09 - Madan
       - Added variable site frequency in Generator
 06/09 - Sasha
       - Hypereconomy calculation is totally redone now.
 06/08 - Sasha
       - Added bias fluctuation by site (the parameters were there but not used)
       - Improved printing of banner data
 06/08 - Madan
...
static long RunModel(int model_id, Site[] siteList,
                     Banner[] bannerList, Category[] catList)
{
    ...
    for (h=0; h < MiscFunc.NUM_ACCESSES; h++) {
      ...
      switch (model_id) {
        ...
        case INTERMEDIATE_ID:
          own_price = bannerList[currentSite].GetIntPrice(1.0);
          break;
      ...
      for (j=0; j < siteList[currentSite].numMatchingSites; j++) {
        bannerToShow = (int) siteList[currentSite].matchingBanners[j];
        matchingCategory = (int) siteList[currentSite].matchingCategories[j];
            ...
            bannerList[currentSite].numLeft > 0 )
        {
          switch (model_id) {
            ...
            case HYPERECONOMY_ID:
              ...
              double cat_estimate = (catList[matchingCategory].catClicks + 1.0 ) /
                      (catList[matchingCategory].catDisplays + 1.2 / MiscFunc.GLOBAL_CLICK_RATE);

              double cat_estimate_average = (catList[matchingCategory].catClicksTotal + 1.0 ) /
                 (catList[matchingCategory].catDisplaysTotal + 1.2 / MiscFunc.GLOBAL_CLICK_RATE);

              double cat_correction_temporal = 0.8;
              if (catList[matchingCategory].catClicksTotal > 10.0)
                 cat_correction_temporal = cat_estimate / cat_estimate_average;

              double site_to_cat_estimate =
                    (siteList[currentSite].siteVsCatClick[matchingCategory]  + 1.0) /
                     (siteList[currentSite].siteVsCatImp[matchingCategory]  + 1.2 /
                     cat_estimate_average);

              double banner_to_cat_estimate =
                    ( bannerList[bannerToShow].banVsCatClick[matchingCategory] + 1.0) /
                     (bannerList[bannerToShow].banVsCatImp[matchingCategory] + 1.2 /
                     cat_estimate_average);

              double site_to_cat_correction = 0.8;
              if (siteList[currentSite].siteVsCatClick[matchingCategory] > 2.0)
                site_to_cat_correction = site_to_cat_estimate / cat_estimate_average;

              double banner_to_cat_correction = 0.8;
              if (bannerList[bannerToShow].banVsCatClick[matchingCategory] > 2.0)
                banner_to_cat_correction = banner_to_cat_estimate / cat_estimate_average;

              double individual_estimate =
                  (bannerList[bannerToShow].siteVsBanClick[currentSite] + 1.0) /
                                (bannerList[bannerToShow].siteVsBanImp[currentSite] + 2.4 /
                           (site_to_cat_estimate + banner_to_cat_estimate));

              double site_banner_correction = 0.8 * site_to_cat_correction *
                                                    banner_to_cat_correction;

              if (bannerList[bannerToShow].siteVsBanClick[currentSite] > 2.0)
                site_banner_correction = individual_estimate / cat_estimate_average;

              utility = cat_estimate_average
                                    * cat_correction_temporal
                                    * site_banner_correction;

/* Old way: ******************************************************************
                  double individual_estimate = (bannerList[bannerToShow].siteVsBanClick[currentSite] + 1.0) /
                                   (bannerList[bannerToShow].siteVsBanImp[currentSite] + 2.0 /
                                   (site_to_cat_estimate + banner_to_cat_estimate);
          ...
          if (bannerList[bannerToShow].siteVsBanClick[currentSite] < 2)
                ...
                utility = individual_estimate * cat_estimate * 0.5 * (1.0 + 1.0 /
                          catList[matchingCategory].catClicksTotal *
                          catList[matchingCategory].catDisplaysTotal);
*******************************************************************/
    ...
    if (bestBannerIndex >= 0) {
       bannerToShow = (int) siteList[currentSite].matchingBanners[bestBannerIndex];
       price       = bestPrice;
       utility     = bestUtility;
       sort_factor = bestSortFactor;
       matchingCategory = (int) siteList[currentSite].matchingCategories[bestBannerIndex];
    ...
    if (utility > 0.9 || h < 10 || currentSite < 3  || bannerToShow < 3)
      ...
      access2Output.println("catVsCat: " + catList[matchingCategory].catClicks
                                 + "/" + catList[matchingCategory].catDisplays);
      ...
      access2Output.println("Adaptive multiplier for category: " +
             (1.0 + catList[matchingCategory].biasAmplitude *
               Math.sin ((double) currentTime /
               catList[matchingCategory].biasPeriod +
             catList[matchingCategory].biasShift)));
    }
      ...
      catList[matchingCategory].catDisplays =
        miscFunctions.incrementImpressions (catList[matchingCategory].catDisplays);
      catList[matchingCategory].catDisplaysTotal += 1.0;
      ...
      if (Math.random() <= MiscFunc.GLOBAL_CLICK_RATE *
         bannerList[bannerToShow].banVsSiteBias[currentSite] *
             ...
             // variable factor for category:
             (1.0 + catList[matchingCategory].biasAmplitude *
               Math.sin ((double) currentTime /
               catList[matchingCategory].biasPeriod +
                 catList[matchingCategory].biasShift)) *
             // variable factor for site:
             (1.0 + siteList[currentSite].biasAmplitude *
               Math.sin ((double) currentTime /
               siteList[currentSite].biasPeriod +
                 siteList[currentSite].biasShift)) *
             // variable factor for banner:
             (1.0 +  bannerList[bannerToShow].biasAmplitude *
               Math.sin ((double) currentTime /
                bannerList[bannerToShow].biasPeriod +
                bannerList[bannerToShow].biasShift)) *
             siteList[currentSite].siteVsCatBias[matchingCategory] *
             catList[matchingCategory].bias)
      ...
      catList[matchingCategory].catClicks =
         miscFunctions.incrementClicks (catList[matchingCategory].catDisplays,
         catList[matchingCategory].catClicks, clickIncrement);
      catList[matchingCategory].catClicksTotal += clickIncrement;
  ...
  try {
    FileOutputStream fout =  new FileOutputStream("CATEGORY." + modelExt);
    PrintStream catOutput = new PrintStream(fout);

    catOutput.println("ID\tclicks\tdisplays\ttotalClicks\ttotalDisplays");
    for (j=0; j < MiscFunc.NUM_CATEGORIES; j++) {
      catOutput.println(j + "\t" +
                        catList[j].catClicks + "\t" +
                        catList[j].catDisplays + "\t" +
                        catList[j].catClicksTotal + "\t" +
                        catList[j].catDisplaysTotal);
    }
  }
  catch (IOException e) {
    System.out.println("Error opening file: CATEGORY." + modelExt + "; " + e);
    System.exit(1);
  }
...
public static void main(String argv[]) {
  ...
  try {
    ...
    for (j=0; j < MiscFunc.NUM_CATEGORIES; j++) {
      ...
      tempVal                         = Integer.valueOf(tempTokenizer.nextToken()).intValue();
      catList[j].bias                 = Double.valueOf(tempTokenizer.nextToken()).doubleValue();
      catList[j].biasAmplitude = Double.valueOf(tempTokenizer.nextToken()).doubleValue();
      catList[j].biasPeriod    = Double.valueOf(tempTokenizer.nextToken()).doubleValue();
      catList[j].biasShift     = Double.valueOf(tempTokenizer.nextToken()).doubleValue();
      catList[j].numSitesPerCat       = Integer.valueOf(tempTokenizer.nextToken()).intValue();
      for (k=0; k < catList[j].numSitesPerCat; k++) {
        catList[j].sitesPerCat[k] = Integer.valueOf(tempTokenizer.nextToken()).intValue();
      }
    }
  }
  catch (IOException e) {
  ...
  for (i=0; i < MiscFunc.NUM_SITES; i++) {
    ...
    for (j=0; j < siteList[i].numCategoriesIn; j++) {
      for (k=0; k < catList[siteList[i].categoriesIn[j]].numSitesPerCat; k++) {
        if (!sitesCheck[catList[siteList[i].categoriesIn[j]].sitesPerCat[k]])
        {
          sitesCheck[catList[siteList[i].categoriesIn[j]].sitesPerCat[k]] = true;
          siteList[i].matchingBanners[siteList[i].numMatchingSites] = (double)
                           catList[siteList[i].categoriesIn[j]].sitesPerCat[k];
          siteList[i].matchingCategories[siteList[i].numMatchingSites] = (double)
                           bannerList[(int)
          siteList[i].matchingBanners[siteList[i].numMatchingSites]].MatchCategory(siteList[i]);
   ...
   if (argv.length == 0) {
     ...
     for (model_id=0; model_id <= NUM_MODELS; model_id++) {
       numModelClicks[model_id] = RunModel(model_id,siteList,bannerList,catList);
     }
   }
   else {
